...
namespace LinqExploration.Projection
{
    ...
    internal class SelectTests
    {
        ...
        {
            var artistNames = from artist in AlbumData.Artists1 select artist.Name;
            var numArtists = AlbumData.Artists1.Count();
        ...
        {
            var tracks = (from artist in AlbumData.Artists1 select from album in artist.Albums select album.Tracks).ToList();
            ...
            Assert.That(tracks.First().Count(), Is.EqualTo(AlbumData.Artists1.First().Albums.Count()));

            Assert.That(tracks.First().First(), Is.AssignableTo<IEnumerable<Track>>());
            Assert.That(tracks.First().First().Count(), Is.EqualTo(AlbumData.Artists1.First().Albums.First().Tracks.Count()));
        }
    }
}
